...
export default class Game {
    constructor(canvas) {
        ...
        this.tutorialSteps = [
            ...
            {
                title: 'Ready to Play!',
                text: 'Defeat all 3 bosses to win. Good luck, little slime!',
                position: 'center',
                highlight: null
            }
        ];
    ...
    startTutorial() {
        ...
        // Start the game in background for tutorial
        this.start();
    ...
    showTutorialStep() {
        ...
        for (let i = 0; i < this.tutorialSteps.length; i++) {
            ...
            if (i < this.tutorialStep) dot.classList.add('completed');
            if (i === this.tutorialStep) dot.classList.add('active');
    ...
    endTutorial() {
        ...
        // If game wasn't started, go back to menu
        if (this.state === 'menu') {
            // Game was already started by startTutorial
        }
        // Game continues from where it is
    }
    ...
    update() {
        ...
        // Apply regeneration (HP per second)
        if (this.player.regeneration > 0 && this.player.health < this.player.maxHealth) {
            const regenAmount = this.player.regeneration * (this.deltaTime / 1000);
            this.player.health = Math.min(this.player.maxHealth, this.player.health + regenAmount);
        }
    ...
    showMutationScreen() {
        ...
        options.forEach((option, index) => {
            ...
            const scaledRegen = option.regeneration ? (option.regeneration * levelMultiplier).toFixed(1) : 0;
            ...
            // Build stats line
            let statsLine = `Health: +${scaledHealth} | Speed: +${scaledSpeed} | Attack: +${scaledAttack}`;
            if (scaledRegen > 0) {
                statsLine += ` | Regen: +${scaledRegen}/s`;
            }
